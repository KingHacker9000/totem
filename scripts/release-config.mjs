...
export async function loadContract() {
  ...
  if (
    parsed?.schema !== "totem.release-config/v1" ||
    !Array.isArray(parsed.variables)
  ) {
    throw new Error(
      "config/release-config.json must use schema totem.release-config/v1",
    );
  }
  const names = new Set();
  for (const entry of parsed.variables) {
    ...
    if (names.has(entry.name))
      throw new Error(`duplicate contract variable: ${entry.name}`);
  ...
  switch (entry.validation) {
    case "port": {
      ...
      if (
        ...
        fail("must be an integer between 1 and 65535");
      break;
    }
    ...
    case "unit-number":
      if (
        ...
        fail("must be a number between 0 and 1");
      break;
    case "environment":
      if (!["development", "test", "production"].includes(value))
        fail("must be development, test, or production");
      break;
    case "log-level":
      if (
        ![
        ...
        fail("has an unsupported log level");
      break;
    case "stt-provider":
      if (!["none", "whisper.cpp"].includes(value))
        fail("must be none or whisper.cpp");
      break;
    case "tts-provider":
    ...
    case "provider-id":
      ...
      if (!/^[a-z0-9][a-z0-9-]*$/.test(value))
        fail("must be a lowercase package/provider id");
      break;
    case "json-object":
      try {
    ...
    case "hostname":
      if (value.length > 253 || /[\s/]/.test(value))
        fail("must be a hostname or IP address without whitespace or slashes");
      break;
...
export function validateEnvironment(contract, env) {
  const byName = new Map(
    contract.variables.map((entry) => [entry.name, entry]),
  );
  const issues = [];
  for (const [name, value] of Object.entries(env)) {
    ...
    if (!entry) {
      issues.push(
        `${name}: unknown TOTEM_ variable; add it to config/release-config.json before use`,
      );
      continue;
    }
...
export async function checkContract() {
  ...
  if (missing.length) {
    throw new Error(
      `runtime/deployment configuration drift: undeclared variables: ${missing.join(", ")}`,
    );
  }
  const example = parseEnvFile(
    await readFile(path.join(root, "deploy/pi/totem.env.example"), "utf8"),
  );
  const issues = validateEnvironment(contract, example);
  if (issues.length)
    throw new Error(
      `invalid credential-free Pi environment example:\n- ${issues.join("\n- ")}`,
    );
  return {
    schema: contract.schema,
    declared: declared.size,
    referenced: references.size,
    credential_free_example: "PASS",
  };
}
...
async function main() {
  ...
  if (command === "validate-env") {
    ...
    const env =
      fileIndex >= 0
        ? parseEnvFile(
            await readFile(path.resolve(args[fileIndex + 1]), "utf8"),
          )
        : process.env;
    const issues = validateEnvironment(contract, env);
    if (issues.length)
      throw new Error(
        `invalid Totem release configuration:\n- ${issues.join("\n- ")}`,
      );
    console.log(
      JSON.stringify(
        { schema: contract.schema, valid: true, credential_free: true },
        null,
        2,
      ),
    );
  ...
  throw new Error(
    `unknown command '${command}'; expected check or validate-env`,
  );
}
