...
export function inspectWorkflowText(text, file = "<workflow>") {
  ...
  for (const [index, rawLine] of text.split(/\r?\n/).entries()) {
    ...
    if (at <= 0) {
      failures.push(`${file}:${index + 1}: external action is missing an immutable ref: ${spec}`);
      continue;
    }
    ...
    if (!SHA40.test(ref)) {
      failures.push(`${file}:${index + 1}: ${action}@${ref} is mutable; pin a 40-character commit SHA`);
    }
  }
...
export function main(argv = process.argv.slice(2)) {
  ...
  const entryCount = results.reduce((sum, result) => sum + result.entries.length, 0);
  const summary = {
  ...
  if (json) console.log(JSON.stringify(summary, null, 2));
    ...
    for (const result of results) {
      console.log(`${result.root}: ${result.workflows} workflows, ${result.entries.length} external actions`);
      for (const entry of result.entries) console.log(`  ${entry.file}:${entry.line} ${entry.action}@${entry.ref}`);
      for (const failure of result.failures) console.error(`  FAIL ${failure}`);
    }
    console.log(`workflow action pinning: ${summary.overall} (${entryCount} actions, ${failureCount} failures)`);
  }
