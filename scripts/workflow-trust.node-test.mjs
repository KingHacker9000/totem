...
test("rejects attacker-controlled checkout refs and inherited secrets", () => {
  ...
  assert.ok(
    result.failures.some((failure) => /secrets: inherit/.test(failure)),
  );
  assert.ok(
    result.failures.some((failure) => /attacker-controlled/.test(failure)),
  );
});
...
test("rejects ambiguous inline triggers", () => {
  ...
  assert.ok(
    result.failures.some((failure) => /inline workflow triggers/.test(failure)),
  );
});
