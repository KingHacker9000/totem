...
function packagePurl(name, version) {
  if (
    ...
    throw new Error(
      "SBOM runtime dependency is missing an exact name/version identity.",
    );
  }
  if (version === "unknown") {
...
export function buildCycloneDx({
  ...
  if (thirdPartyManifest?.schema !== "totem.third-party-release-manifest/v1") {
    throw new Error(
      "SBOM requires a totem.third-party-release-manifest/v1 manifest.",
    );
  }
  if (bundleManifest.repository !== thirdPartyManifest.source?.repository) {
    throw new Error(
      "SBOM source repository identity differs between release and third-party manifests.",
    );
  }
  ...
  if (thirdPartyManifest.boundaries?.publicReleaseOnly !== true) {
    throw new Error(
      "SBOM third-party dependency boundary is not marked public-release-only.",
    );
  }
  if (
    ...
    throw new Error(
      "SBOM third-party manifest does not record the private Portal exclusion boundary.",
    );
  }
  ...
  if (!Array.isArray(runtime)) {
    throw new Error(
      "SBOM third-party manifest is missing runtime dependency candidates.",
    );
  }
  ...
  const components = runtime
    .map((entry) => {
      ...
      if (seen.has(purl)) {
        throw new Error(`Duplicate SBOM runtime component: ${purl}`);
      }
      seen.add(purl);
      return {
        type: "library",
    ...
    metadata: {
      component: {
        ...
        properties: sortedProperties([
          ...
          {
            name: "totem:source-revision",
            value: bundleManifest.source.revision,
          },
          { name: "totem:source-tree", value: bundleManifest.source.tree },
          {
            name: "totem:release-bundle-sha256",
            value: bundleManifest.digest.value,
          },
          {
            name: "totem:lockfile-sha256",
            value: thirdPartyManifest.source.lockfileSha256,
          },
          {
            name: "totem:third-party-manifest-sha256",
            value: sha256(thirdPartyText),
          },
      ...
      properties: sortedProperties([
        ...
        {
          name: "totem:runtime-scope-source",
          value: "totem.third-party-release-manifest/v1",
        },
        {
          name: "totem:artifact-identity-source",
          value: "totem.public-release-bundle/v1",
        },
      ]),
    },
    components,
...
export function assertSbomMatches(expected, actual) {
  ...
  if (canonicalJson(expected) !== canonicalJson(actual)) {
    throw new Error(
      "Release SBOM is stale or does not match the exact current release artifact/dependency boundary.",
    );
  }
}
...
async function buildCurrent({
  root,
  bundleOutput = DEFAULT_BUNDLE,
  output = DEFAULT_OUTPUT,
}) {
  const bundleManifest = await buildReleaseBundle({
    root,
    output: bundleOutput,
  });
  const thirdPartyPath = resolve(
    root,
    "dist/release/third-party-manifest.json",
  );
  ...
  const thirdPartyBytes = Buffer.from(
    canonicalJson(thirdPartyManifest),
    "utf8",
  );
...
async function main() {
  ...
  if (command !== "generate" && command !== "verify") {
    throw new Error(
      "usage: release-sbom.mjs <generate|verify> [--output <path>] [--bundle <path>]",
    );
  }
