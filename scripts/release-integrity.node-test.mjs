...
async function fixture() {
  ...
  await writeFile(
    resolve(bundle, "release-manifest.json"),
...
test("generates deterministic offline-verifiable manifest", async () =>
  withFixture(async (f) => {
    ...
    assert.equal(
      (await verifyIntegrity(f)).releaseBundleDigest,
      "2".repeat(64),
    );
  }));

test("rejects tampered, missing, and unexpected files", async () =>
  withFixture(async (f) => {
...
test("rejects duplicate and unsafe manifest paths", async () =>
  withFixture(async (f) => {
    ...
    await assert.rejects(
      () => verifyIntegrity(f),
      /duplicate integrity manifest path/,
    );
    ...
    await assert.rejects(
      () => verifyIntegrity(f),
      /unsafe integrity manifest path/,
    );
  }));

test("rejects release identity drift even when file checksums are refreshed", async () =>
  withFixture(async (f) => {
    await generateIntegrity(f);
    ...
    await assert.rejects(
      () => verifyIntegrity(f),
      /integrity mismatch|does not match embedded release identity/,
    );
  }));
