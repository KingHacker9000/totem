...
test("rejects artifact digest drift without source changes", async () => {
  await withFixture(async (root) => {
    ...
    await assert.rejects(
      () => verifyProvenance({ root, output: "provenance.json" }),
...
test("rejects private Portal artifact paths", async () => {
  await withFixture(async (root) => {
    ...
    await writeFile(
      join(root, "totem-portal-theme", "private.bin"),
      "private",
      "utf8",
    );
    await assert.rejects(
      () =>
        generateProvenance({
          root,
          artifacts: ["totem-portal-theme/private.bin"],
          output: "provenance.json",
        }),
