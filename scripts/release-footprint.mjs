...
export function validateBudget(raw) {
  if (raw?.schema !== BUDGET_SCHEMA) {
    throw new Error(`unsupported footprint budget schema: ${raw?.schema}`);
  }
  ...
  for (const [index, entry] of exceptions.entries()) {
    if (
      ...
      throw new Error(
        `exceptions[${index}].path must be one exact safe bundle-relative path`,
      );
    }
    if (seen.has(entry.path)) {
      throw new Error(`duplicate footprint exception path: ${entry.path}`);
    }
    seen.add(entry.path);
    positiveInteger(entry.maxBytes, `exceptions[${index}].maxBytes`);
    if (typeof entry.reason !== "string" || entry.reason.trim().length < 12) {
      throw new Error(
        `exceptions[${index}].reason must explain the bounded exception`,
      );
    }
  }
  return {
    ...
    topContributors: positiveInteger(
      raw.topContributors ?? 10,
      "topContributors",
    ),
    exceptions,
  };
...
export function evaluateFootprint(files, budget) {
  ...
  const exceptionMap = new Map(
    budget.exceptions.map((entry) => [entry.path, entry]),
  );
  const violations = [];
  if (totalBytes > budget.maxTotalBytes) {
    violations.push(
      `total bytes ${totalBytes} exceeds budget ${budget.maxTotalBytes}`,
    );
  }
  if (sorted.length > budget.maxFileCount) {
    violations.push(
      `file count ${sorted.length} exceeds budget ${budget.maxFileCount}`,
    );
  }
  ...
  for (const exception of budget.exceptions) {
    if (!sorted.some((entry) => entry.path === exception.path)) {
      violations.push(
        `footprint exception path is stale or absent: ${exception.path}`,
      );
    }
  }
  return {
...
export async function checkFootprint({
  root = process.cwd(),
  ...
  const budget = validateBudget(
    JSON.parse(await readFile(resolve(root, budgetPath), "utf8")),
  );
  ...
  if (metrics.violations.length) {
    throw new Error(
      `release footprint budget failed:\n- ${metrics.violations.join("\n- ")}`,
    );
  }
  return report;
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const value = argv[i];
    if (!["--bundle", "--budget", "--report"].includes(value)) {
...
async function main() {
  ...
  console.log(
    `${report.status} ${report.metrics.totalBytes} bytes ${report.metrics.fileCount} files; largest ${report.metrics.largestFileBytes} bytes (${report.metrics.largestFilePath})`,
  );
}
