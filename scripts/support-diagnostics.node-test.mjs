...
import { assertNoKnownSecretValues, buildSupportBundle, redactString } from "./support-diagnostics.mjs";
...
test("support bundle exposes configuration shape but never environment values", async () => {
  ...
  const bundle = await buildSupportBundle({ env, baseUrl: "http://127.0.0.1:9" });
  ...
  assert.ok(!serialized.includes("3999"));
