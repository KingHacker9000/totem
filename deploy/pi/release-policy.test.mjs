...
const release = (name, mtimeMs) => ({
  path: `/opt/totem/releases/${name}`,
  mtimeMs,
});
...
describe("Pi release retention", () => {
  it("protects the active release and newest rollback while pruning older releases", () => {
    ...
    const result = chooseRetention({
      releases,
      current: releases[1].path,
      retain: 2,
    });
    ...
    expect(result.prune.map((entry) => entry.path)).toEqual([
      releases[2].path,
      releases[3].path,
    ]);
  });
  ...
  it("never permits retention below current plus one rollback", () => {
    expect(() =>
      chooseRetention({ releases: [], current: null, retain: 1 }),
    ).toThrow(/at least 2/);
  });
});
  ...
  it("passes when free space covers release estimate plus reserve", () => {
    expect(
      assertFreeSpace({
        ...
        minFreeBytes: 2048 * MIB,
        estimatedReleaseBytes: 1024 * MIB,
      }),
...
describe("release policy configuration", () => {
  it("uses fallback for an unset value and rejects invalid integers", () => {
    ...
    expect(() => parsePositiveInteger("nope", 2, "retention")).toThrow(
