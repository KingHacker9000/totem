...
async function fixture() {
  ...
  const serviceFile = path.join(root, "etc", "systemd", "system", "totem.service");
  ...
  await writeFile(path.join(stateDir, "backups", "b1", "manifest.json"), "{}\n");
...
test("default decommission removes runtime/service but preserves state, backups, and config", async () => {
  ...
  const result = await performDecommission({ ...f, runSystemctl: systemctl.run });

  assert.equal(await exists(f.prefix), false);
  assert.equal(await exists(f.serviceFile), false);
  assert.equal(await readFile(path.join(f.stateDir, "state.db"), "utf8"), "durable\n");
  assert.equal(
    await readFile(path.join(f.stateDir, "backups", "b1", "manifest.json"), "utf8"),
    "{}\n",
  );
  assert.equal(await readFile(path.join(f.configDir, "totem.env"), "utf8"), "TOKEN=secret\n");
  assert.equal(result.state, "preserved");
  assert.equal(result.config, "preserved");
  assert.deepEqual(systemctl.calls, ["stop totem.service", "disable totem.service", "daemon-reload"]);
...
test("dry-run makes no filesystem or systemctl changes", async () => {
  const f = await fixture();
  const systemctl = systemctlRecorder();
  const result = await performDecommission({ ...f, dryRun: true, runSystemctl: systemctl.run });
  assert.equal(await exists(f.prefix), true);
  assert.equal(await exists(f.serviceFile), true);
  assert.equal(await exists(f.stateDir), true);
  assert.deepEqual(systemctl.calls, []);
  assert.ok(result.actions.every((item) => ["planned", "preserved"].includes(item.status)));
});
